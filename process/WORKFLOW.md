...
**Multi-item jobs are report-first** (adopted 2026-08-22 from the swallowed-report brief — the measured failure: the work always completed; the report, sent last, was the deliverable that died). For a job of many writes (batch API calls, sweeps, surveys): the status/survey table is sent **before** anything is applied, as its own message; applied items lead with a caller-specified idempotency marker so a resumed or re-dispatched run detects prior work and never double-applies; a turn never ends mid-run without a one-line position report ("stopped after item 7 of 14"); and a failed write is reported with its verbatim error immediately, never silently retried. The companion addressing rule — report to the inbound `teammate_id`, never `"main"`, and a failed send is an undelivered finding — lives verbatim in every agent definition, which is the file the composing agent has always read.
...
**Parallel major maintenance (`V1.x` + `V2.x`).** When breaking work starts, create `majors/V2.md`; now two majors are `active`. `main` tracks the **highest active major**; each prior major gets a long-lived **`release/N.x` branch**. Tags are cut from the matching branch (`v1.4.2` from `release/1.x`, `v2.1.0` from `main`); the flat tag namespace disambiguates automatically. A `V1` bugfix is an issue on a `V1` milestone (PATCH bump only); a `V2` feature is an issue on a `V2` milestone. `majors/V1.md` moves to `status: completed` when you EOL the `1.x` line.
