...
**Lifecycle (PT-86): the receiver starts with the first session on the repo and stops with the last — never mid-way, so one teammate's `/exit` never drops another's live telemetry.** Every `SessionStart` (main session or teammate — each is its own process, so each counts) registers its session id in `process/cairn/metrics/.sessions/` (one small file per id, holding a liveness-probe pid — `$PPID`, the hook shell's own parent, measured to be the Claude Code session process itself); every `SessionEnd` deregisters it. A background watchdog thread inside the receiver, not the short-lived hook-invoked CLI processes, owns the actual decision: when the registry drains from non-empty to empty it arms a grace window (`--grace-period-seconds`, default 10s — long enough for the OTel exporter's own final export to land), cancels it if a new session registers before the window elapses, and otherwise flushes, removes its pidfile, and exits. A liveness probe (`os.kill(pid, 0)` **and** the session's own transcript file going quiet for 30+ minutes — both signals, so a single mis-detected pid can never silently cut off a live session's telemetry) reaps a session that crashed without ever sending its `SessionEnd`, so a dead session can never pin the receiver running forever. `--stop` still works for manual use; `--status` reports the live-session count and each id's liveness (`alive`/`dead`/`unknown` — the last for a session whose pid could never be captured, never reaped by the probe). **Measured (2026-09-04): whether a teammate's `SessionEnd` hook actually fires on session teardown (e.g. a `shutdown_request` approval) is recorded as a PT-86 issue comment** — if it turns out not to, the liveness probe above is the backstop that keeps the receiver's session count honest regardless.
