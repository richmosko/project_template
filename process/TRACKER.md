...
**Adopter remedy, same shape as PT-86's:** after pulling PT-84, restart any already-running receiver once (`python3 scripts/cairn/otel_receiver.py --stop`, then let the next session's `--ensure-running` bring it back) so the daemon picks up milestone-aware attribution for lines it flushes from that point forward — a receiver process already running in memory does not hot-reload the new `resolve_issue`/`milestone_windows` logic just because the code on disk changed underneath it.
