...
**General rule, established twice now and worth stating once: an attribution-logic change is retroactive for `source: "transcript-backfill"` lines and prospective-only for `source: "otel"` lines.** A backfill re-run re-derives every `transcript-backfill` line from the raw transcripts under the new rule, so historical data is corrected. A live `otel` line, once flushed, keeps whatever attribution it was given at flush time permanently — the receiver only appends, and the identity/context needed to re-derive an already-committed line (a role's `session.id`, a record's own original timestamp-to-milestone mapping) is either discarded at flush by design (PT-87, the privacy allow-list) or simply never re-visited by anything downstream. PT-87 hit this first: three spawn-named `otel` lines, flushed before that ruling landed, can never be corrected. PT-84 hits it again: the 2026-09-04 AC 2 re-run left **6** pre-existing `otel` `main` lines dated on/after the first milestone's creation — each is *repairable in principle* (single-day windows, wholly inside one milestone's span, no straddling) but was deliberately **left as `main`, not repaired**, per the architect's call: six lines don't justify a one-off migration whose method only generalizes to the single-day case, and a repair invites reuse where it isn't sound. Say "repairable but not repaired" for these, not "will resolve on its own" — restarting a receiver changes what *future* lines get flushed as; it does not, and cannot, touch a line already on disk.

**Adopter remedy, same shape as PT-86's — and precisely what it does not do.** After pulling PT-84, restart any already-running receiver once (`python3 scripts/cairn/otel_receiver.py --stop`, then let the next session's `--ensure-running` bring it back) so the daemon picks up milestone-aware attribution for lines it flushes from that point forward — a receiver process already running in memory does not hot-reload the new `resolve_issue`/`milestone_windows` logic just because the code on disk changed underneath it. **This prevents further mis-bucketed lines; it does not fix existing ones** — say both halves, not just "restart to fix it", which is false in the half that matters.

**Separately, always stop the receiver before re-running the backfill, and restart it bare (no session id) afterward — say why, not just the step.** A backfill re-run moves the data file's `generated` stamp forward, past whatever the daemon has already buffered; PT-77 §2's non-overlap guard in `flush()` then refuses any batch whose earliest datapoint predates that new stamp, since those datapoints are inside transcripts the regen just counted and flushing them would double-count. Measured during PT-84's own AC 2 run (architect's ruling, 6304afd): this refusal does not merely skip one flush — the guard's `raise` sits before the pending-state reset in `flush()`, so a refused flush leaves the buffer untouched and **every subsequent flush is refused identically for the life of the process**, silently, with everything accumulated lost at exit. Datapoints received after the regen's own `generated` stamp are refused too (the batch-level guard keys on the batch's earliest datapoint) and are **not** recoverable from the backfill, which read transcripts at that instant and cannot contain anything later — real data was lost this way during PT-84's own run. Filed as PT-89 (drop only the pre-`generated` datapoints and flush the remainder, rather than refusing the whole batch) — until that lands, stopping the receiver before a backfill re-run and restarting it bare afterward is the only way to avoid this.
