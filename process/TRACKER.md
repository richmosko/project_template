...
**Lifecycle (PT-86): the receiver starts with the first session on the repo and stops with the last — never mid-way, so one teammate's `/exit` never drops another's live telemetry.** Every `SessionStart` (main session or teammate — each is its own process, so each counts) registers its session id in `process/cairn/metrics/.sessions/` (one small file per id, holding a liveness-probe pid — `$PPID`, the hook shell's own parent, measured to be the Claude Code session process itself); every `SessionEnd` deregisters it. A background watchdog thread inside the receiver, not the short-lived hook-invoked CLI processes, owns the actual decision: when the registry drains from non-empty to empty it arms a grace window (`--grace-period-seconds`, default 10s — long enough for the OTel exporter's own final export to land), cancels it if a new session registers before the window elapses, and otherwise flushes, removes its pidfile, and exits. A liveness probe (`os.kill(pid, 0)` **and** the session's own transcript file going quiet for 30+ minutes — both signals, so a single mis-detected pid can never silently cut off a live session's telemetry) reaps a session that crashed without ever sending its `SessionEnd`, on the next `end` event or at each flush. If **every** registered session crashes without ever ending cleanly, neither trigger fires, and the stale entries wait for an unrelated future session's own `SessionEnd` to clear them — a dead session can pin the receiver for at most one session lifecycle, never longer, and a pin costs only an idle localhost process (§0 of the architect's gating ruling on PT-86: a false pin is cheap, a false reap silently costs a live session's telemetry, so every design choice here breaks toward not stopping). `--stop` still works for manual use; `--status` reports the live-session count and each id's liveness (`alive`/`dead`/`unknown` — the last for a session whose pid could never be captured, never reaped by the probe). **Measured (2026-09-04, PT-86): a teammate's `SessionEnd` hook fires on `shutdown_request` and its hook input carries the session id, so a teammate's exit deregisters it directly; the liveness probe is the backstop for crashed sessions only. Teammate registration on `SessionStart` was confirmed in the same run.**

**Upgrading an existing checkout to PT-86 (one-time, not steady state):** a receiver already running before this change has no handler for the nudge signal PT-86 introduces — run `python3 scripts/cairn/otel_receiver.py --stop` once after pulling, before the next session ends, so the old daemon's existing `SIGTERM` handler flushes cleanly instead of being torn down mid-flush by something it doesn't understand. Separately: any session that was already running when the upgrade landed started before the `SessionEnd` hook line existed, so it is not in the new registry — if a session that starts *after* the upgrade then starts and ends, the registry can go 1 → 0 and the receiver can stop while those older sessions are still live, and their exports go nowhere until their own sessions eventually turn over. If `--status` shows the receiver stopped while sessions are still live, restart it bare (`python3 scripts/cairn/otel_receiver.py --ensure-running`, no session id) — a registry that has never been non-empty never arms the shutdown timer, so it stays up until the next natural session cycle picks it back up correctly.
