...
|---|---|---|---|---|
| SC4 (2026-08-20) | 0.5 | Finish 0.5 through PT-2: PT-19 (title-shape lint) → PT-16 (board names + collapse) → PT-4 (drawer markdown) → PT-2 (snapshot appendix). PT-3 gated on a design conversation; milestone close (v0.5.0) is a planned stop. Standing order (Mosko, 2026-08-20): self-merge authorized up to milestone end. | Current | **All buildable 0.5 features shipped this session: PT-19 (#49), PT-16 (#50), PT-4 (#51), PT-20 (#52), PT-2 (#53).** Two issues remain open in 0.5: PT-3 (multi-root board — gated on a design conversation with Mosko, not started) and PT-21 (cairn ls lexicographic-sort bug, P3 backlog, filed this session). STOPPED at PT-3 per plan — awaiting Mosko on the PT-3 design conversation and the v0.5.0 milestone-close decision. Notable: PT-4 pulled seceng onto the team (marked+DOMPurify, sanitize-after-parse, USE_PROFILES hardening; supply chain byte-verified vs upstream); seceng's Validate review of PT-4 found a pre-existing pr-link XSS → PT-20 filed & shipped same session (DOM-build + http(s) scheme allowlist). Anchor-task pattern ran clean all session (QA-owned anchors, blockedBy subtasks, live SendMessage). Validate split for UI: QA suite+read-review, team-lead Chrome pass (QA has no browser MCP). |
