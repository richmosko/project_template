...
|---|---|---|---|---|
| SC2 (2026-08-20) | 0.4 | 0.4 scoped (12 issues; PT-2/3/4 deferred to 0.5). Order: PT-15 (STATE.md rename) → PT-14 (Linear removal) → hardening batches (PT-5/6/12, PT-7/8/9/13, PT-10/11) → PT-1 (SSE live push). | Current | Scope decision logged in TEMPLATE_DECISIONS.md (2026-08-20). PT-15 (#34), PT-14 (#35), PT-5 (#36) shipped — 0.4 at 3/13 (PT-17 pulled in: coordination-toolset fix, ratified with capability grant). PT-16 filed (board name display). Teammate SendMessage down all session (allowlist omission found, but docs say auto-add — session-level cause suspected; PT-17 tests it); temp/ fallback held. QA corrected its PDF-check method (byte-grep → pdftotext, on PT-15) and caught an orphaned live-data cairn serve on :18766 (killed). |
