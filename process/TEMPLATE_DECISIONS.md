...
### 2026-09-05 — Loops run as four gates with mechanical caps (PT-94)
**Decision:** Every Implement→Validate loop is four gates — ruling, tests red, build green, verdict — each one issue-file commit and one message; the caps (one report per turn ≤ 8 lines, comments ≤ 40 lines, issue files ≤ 24 KB, one full-suite run per gate by its owner, ≤ 40 messages / 3 ruling sections / 15 commits / 4 full runs per loop) bind in the agent definitions and are enforced by a `SendMessage` hook, a pre-commit foreign-hunk guard, `cairn check` warnings, `cairn gate --head`, and a `cairn loop-stats` scorecard pasted into every PR.
**Why:** Measured step by step on the PT-85 loop (`process/reviews/PT-94/`): 16 full-suite runs where 3 were needed, 33 of the lead's 64 inbound messages idle notifications (19 duplicating the report they followed), 16 of the lead's 56 messages relays of file decisions, 12 ask-and-wait round trips, and a verdict issued in five parts; PT-84's issue-file commits scored the same shape (27 commits, 10 outcome-changing, 7 parts of one verdict, 7 changing nothing). The waste was in the loop's shape, and every prior fix by reminder message had been measured to decay within the same loop.
**Alternatives considered:**
- Reminder messages at spawn — rejected: the PT-85 loop already ran under Loop discipline rules 1–8 and produced the numbers above; a rule that binds only while remembered is not a rule.
- Fewer roles per loop (architect reviews only at the end) — deferred: the four-gate flow already restricts the architect to two commits; role count is a separate decision if the scorecard still shows drift.
- Hard blocks for every cap — rejected for comments and issue size (a warning the lead acts on at the next gate; a hard block would refuse a legitimate long verdict table mid-gate), kept for messages (the pointer form has no legitimate long case) and for two-author commits (never legitimate).
**Approved by:** Rich Mosko (directive 2026-09-05: audit the transcripts, fix the cause in the definitions; the lead executes PT-94 directly). Ruling and implementation team-lead 2026-09-05.
**Supersedes:** extends the 2026-09-05 Loop discipline (eight rules) with the four-gate flow and the caps; the "commit by pathspec" rule's soft wording ("read the diff") is replaced by the mechanical guard.

### 2026-09-02 — STATE.md's Releases section becomes a one-row pointer, machine-bounded at 200 characters (PT-75)
**Decision:** `process/STATE.md` → `## Releases` holds exactly **one** data row — the latest tagged release — hard-capped at ≤ 200 characters (outer pipes + link URL included), enforced by a new test, `scripts/cairn/tests/test_state_releases_bound.py`, which `/finish-feature` already runs as a hard gate. `/merge-pr` now **replaces** that row on every tag instead of appending; a fixed row template (version, date, major line, milestone shipped, branch, Draft|Published) is quoted identically in `process/WORKFLOW.md` → Release process and `.claude/skills/merge-pr/SKILL.md`. The 11 rows this collapses (v0.3.0 → v0.9.0) are not lost: each removed row's synthesis prose (issue counts, PR ranges, per-issue summaries, breaking-change notes) was verified per-version against its GitHub Release body and ported in via `gh release edit` wherever not already present (9 of 11 needed porting; v0.6.0 and v0.7.1 already carried equivalent synthesis). `git show 5b93de9:process/STATE.md` is the permanent backstop copy of the pre-collapse table.
**Why:** The section's own header promises "**No history accumulates here**," but the Releases table had grown into an append-only log that contradicted it outright — measured on `5b93de9`: 11 rows, 8489 bytes, 83.7% of the whole file. A doc that contradicts itself teaches agents to distrust it. The full-read cost is real and paid often: `CLAUDE.md`'s Resume runbook step 1 directs "**Read the full `process/STATE.md`**" on every cold session resume — exactly the moment a lead is trying to orient quickly, and 83.7% of that read was duplicated release history. **Corrected premise, not carried forward:** the original ruling's motivation cited SessionStart injection cost, but the hook's `awk` extractor already stops at the `## Releases` heading (`.claude/settings.json`), so the table was never part of any session's auto-injected context — that claim was false and is deliberately not repeated here.
**Alternatives considered:**
- Per-row size cap ("one line per release") — rejected: prose fields regrow with no enforceable bound, the same failure mode the 2026-08-22 Session Cycles retirement already ruled out; a hard row-count cap is the only durable discipline.
- Trusting "GitHub Releases already carries this verbatim" as grounds to delete the rows unread — rejected as a false premise: spot-checking v0.9.0 showed GitHub's `--generate-notes` body is PR-list-only, with none of STATE.md's synthesis prose. Each row's synthesis was verified and ported per-version instead of assumed.
**Approved by:** Rich Mosko (delivery pre-authorized 2026-09-02: merge PT-0.10 issues, then release v0.10.0); ruling + acceptance criteria team-lead 2026-09-02; two premise corrections (session-startup cost, recoverability) architect 2026-09-02.
**Supersedes:** the "Releases table kept" clause of the 2026-08-19 Resolutions-log entry (row 1: "`STATE.md` keeps Current Phase, Active Feature, Session Cycles, Releases, plus a board pointer...") and the 2026-08-22 "STATE.md keeps no history" entry's Decision line ("...it holds only current state (Current Phase, Active Feature pointer, **Releases table**)"). Both treated "Releases" as an already-bounded current-state field; by 2026-09-02 it had grown unboundedly instead. This entry makes that characterization true by making the row itself bounded, rather than retracting either prior ruling's broader point (STATE.md stays a dashboard, not a log).

### 2026-08-31 — The dashboard is the root entry point; the standalone board gets /board (PT-73)
**Decision:** Bare `/` 302-redirects (with `Cache-Control: no-store`) to `/dashboard` whenever `dashboard/dist/index.html` exists — the shell is the primary interface (Mosko). Carve-outs preserve standing rulings: embed-param requests (`?embed=1…`) always serve the board (the PT-55 recursion guard — the shell's own iframes load `/`), and a missing or half-cleaned dist serves the board unchanged (zero-build clone fallback; the predicate is `.is_file()` on index.html, shared by `/` and `/dashboard` so they cannot drift). `/list` stays a deep-link; the standalone kanban view gets its own never-redirecting `/board` alias (without it, the board's own Kanban tab (`href="/"`) would have silently bounced standalone users into the dashboard whenever dist existed).
**Why:** Post-PT-72 there were three URLs with inconsistent chrome; Mosko ruled the root should land on the unified shell. 302+no-store because the predicate is mutable filesystem state — a cached permanent redirect would keep firing into a 503 after `dist/` is deleted, unfixable server-side.
**Approved by:** Mosko (root-loads-dashboard, 2026-08-31); architect mechanics ruling (PT-73 comment ae35fc1) accepted by team-lead incl. the /board alias.

### 2026-08-29 — Preset-purity relaxes to preset-as-default + user-selectable shadcn-native variants (PT-69)
**Decision:** The design system's 2026-08-26 "preset-pure — no blending, no invented hues" ruling relaxes: preset `b6XadDxmQS` (Stone/Sky/Yellow) remains the DEFAULT, and users select among shadcn's own published variants via a theme-settings dropdown on both surfaces — Base ×7 (the OKLCH generator's complete set), Theme ×23 (full set minus `lime`, dropped by the per-variant contrast fence), Chart ×24 (each with its own derived, validated `--chart-flow-*` ordinal ramp), Mode 3-state (system|light|dark, default system). Variants ship as vendored generator data (`design/variants.json`) + a stdlib generator emitting per-surface CSS; applied via `data-cairn-*` attributes; one origin-global `cairn.theme` localStorage key with a generated inline head bootstrap on both surfaces (no FOUC; per-dimension fallback). Derived-ink deviations from the preset (muted-foreground per base, destructive-foreground split per mode, yellow theme's primary-foreground) exist solely to clear the 4.5:1 floor on rendered pairs — gates hold with zero exemptions.
**Why:** Mosko wants per-user theme/color control mirroring shadcn's own settings menu; honest accessibility outranks literal preset fidelity (three separate ink derivations were each ruled over accept-and-record).
**Alternatives considered:** mode-toggle-only (rejected by Mosko); curated subsets (rejected — full reference lists requested); accepting sub-AA contrast as recorded debt (rejected each time).
**Approved by:** Mosko (scope 2026-08-28/29; darken-the-ink and derive-to-pass rulings 2026-08-29); architect rulings on the issue (variant architecture, gate semantics, per-mode ink split); ux-designer option sets + interaction spec.

### 2026-08-29 — Theme choice is origin-global, deliberately departing from PT-30's repo-scoped keys (PT-69)
**Decision:** `cairn.theme` is one origin-global localStorage key, not repo-scoped like PT-30's `expandedLanes`.
**Why:** A board's expansion state is a fact about a *project*; a theme is a fact about the *person looking* — reverting someone's theme when they open a different repo's board on the same port would be the bug. Cost if wrong: a one-line key change plus a migration read.
**Alternatives considered:** repo-scoped key (wrong dimension for this datum).
**Approved by:** architect ruling (PT-69, 2026-08-29), accepted by team-lead.

### 2026-08-29 — Two-model contrast rule with ε=0.05 margin, repo-wide (PT-69)
**Decision:** Every derived color token must clear its contrast floor under BOTH an 8-bit-hex-quantized and a continuous-float OKLCH model, with ≥0.05 margin. The margin floor is enforced by a derivation-bypass detector test (a value under 0.05 can only mean a hand-edit that skipped regeneration or a broken generator — never a threshold to loosen). Per-mode identity of a token is a result of validation, never an input assumption.
**Why:** Two independently-written contrast bridges disagreed by ~0.8% (float vs quantized) with the floor inside the gap — an already-merged derived value failed the model it hadn't been checked against. The generator and its guard shared one color bridge and therefore one blind spot; independence is the fix. Surfaced only because ux-designer wrote her own implementation and reported a sub-1% disagreement instead of rounding it away.
**Alternatives considered:** single-model + tighter floor (still one blind spot); trusting either model alone.
**Approved by:** architect ruling (PT-69 commit 4e0e678), 2026-08-29. General repo rule, not a PT-69 detail — it invalidated a value that had already merged.

### 2026-08-28 — Issue-flow history is reconstructed from git blobs; dashboard takes its first real dependency tree (PT-61)
**Decision:** The dashboard's issue-flow-over-time chart sources its history by reconstructing per-day status counts from git (`git log --raw -M` change events + one `git cat-file --batch` read as bytes, through the engine's existing frontmatter parser), served by a separate `GET /api/flow` endpoint memoized on the full HEAD sha (pure ref-file read; zero subprocesses on cache hit), degrading to `series: []` + warning with HTTP 200. No snapshot/persistence layer. With the chart panel, the dashboard takes its first real dependency tree: `bits-ui` (Sidebar), `layerchart` + d3 (Charts) — bundle steps up from 131KB with the chart as its own dynamically-imported chunk. The chart's counts legitimately differ from the live status cards (committed-only, archived-included); the panel labels its scope rather than splicing live state. The chart's six series colors are a re-stepped chart-local golden ramp (`--chart-flow-*`), validator-passed against both surfaces, because the preset's own `--chart-1/2` fail the ordinal light-end contrast floor on white; the preset tokens remain untouched.
**Why:** No historical data existed anywhere; git is the only retroactive source and is already the engine's trust anchor. Diff-scraping was rejected as a third parser of the file format that body text can fool. Honest divergence over fabricated agreement, same principle as the PT-56 roster.
**Alternatives considered:** periodic snapshot appending (no backfill, new moving part); splicing live tree state onto the final chart point (fabricates a committed state that doesn't exist); using the preset ramp as-is (validator hard-fail); folding statuses to 3 bands (loses granularity).
**Approved by:** architect ruling (PT-61 comment 1eae1b2) accepted by team-lead; ramp re-step and full-sha cache key ruled/acked on the issue; dependency adoption approved by Mosko in the PT-61 feature plan (2026-08-28).

### 2026-08-28 — Agent-roster presence: no synthesized source; honest working/idle/unknown from tracker assignments (PT-56)
**Decision:** The dashboard's agent roster derives identity from `.claude/agents/*.md` + `.claude/roles/team-lead.md` and work attribution from live issues' `assignee`; presence is strictly `working` (live in-progress/in-review assignment) / `idle` (backlog/todo, stale — issue `updated:` predating today, surfaced as `stale_since` — or done-only) / `unknown` (no data), never "active"/"online"/"live". Served by `GET /api/roster` as a composed reader module — not an engine change, not a key on `/api/dashboard`. A legend states verbatim: "Derived from tracker assignments, not live presence — nobody is being pinged." Degradation is one-directional; a done assignment renders only as history ("last shipped"). Process implications adopted with it: claiming an issue sets `assignee` (populating the panel with real data), and `assignee` is deliberately **preserved** on done issues as provenance.
**Why:** No reliable presence source exists (architect ruling, measured 2026-08-28): tmux is unreachable from agent context and contingent on a settings toggle; the shared task store carries no timestamps and *deletes* completed tasks (an idle agent would be absent, not idle); assignee fields were universally null until the claim-time change. Fabricating liveness would violate the AC's honesty clause; the empty state (a cloner's eleven cards, all unknown) is the roster the template actually provides.
**Alternatives considered:** tmux pane liveness, task-store mtime, a per-turn heartbeat hook — heartbeat is the designated upgrade path, pending a spike on whether hooks fire for spawned teammates (refused to rule unverified).
**Approved by:** architect ruling (PT-56 comments 012a105, 9728557, 704e711), accepted by team-lead; preserve-on-done and claim-sets-assignee by team-lead 2026-08-28.
...
**Alternatives considered:**
- Vanilla lookalike on the zero-build board stack — keeps the template dependency-free but re-implements or skips what shadcn ships.
- Hybrid was effectively adopted implicitly: only the dashboard is Svelte; the board's vanilla stack is untouched this milestone.
**Approved by:** Mosko (2026-08-26, choosing "Real shadcn-svelte" over lookalike/hybrid options).
**Supersedes:** the board's "no framework, no build step" principle as a template-wide rule — it now scopes to the vanilla board surfaces only.
