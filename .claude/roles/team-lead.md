...
**Idle-without-report escalation ladder** (adopted 2026-08-22; the swallowed-report class — work done, report dead): (1) check `temp/` first; (2) read the idle notice's own summary line for a **`[to main]` prefix** — that is a swallowed report, not a stalled agent: the work is likely done and re-dispatching risks double-application; (3) one diagnostic poke ("reply with just the row you're on"); (4) if bare idles continue, **read ground truth directly** from the tree or target system rather than poking again — three direct reads settle what five pokes cannot; (5) any replacement instance is briefed **survey-first**, so partial prior work is detected rather than assumed absent.
