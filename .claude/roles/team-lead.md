...
**Idle-without-report ladder** (ruled 2026-08-22; provenance in `TEMPLATE_DECISIONS.md`): (1) check `temp/`; (2) read the idle notice's summary line — a **`[to main]` prefix** means the report was swallowed: assume the work is done, do not re-dispatch; (3) one diagnostic poke ("reply with just the row you're on"); (4) then **read ground truth directly** from the tree or target system instead of poking again; (5) brief any replacement instance **survey-first**.
