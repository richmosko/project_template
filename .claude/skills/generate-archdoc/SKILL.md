...
description: Generates or refines `docs/ARCH/index.html` — the Architecture & Infrastructure document. Use during the Plan phase, or whenever the user asks to "write the architecture doc", "design the system", or "diagram the components". Driven by the `architect` agent. Produces a doc with System Context, Components, Data Flow (Mermaid), Tech Stack & Rationale, Deployment Topology, Integration Points, Trade-offs, and Open Questions. **Supports import mode**: pass an optional path to an existing ARCH artifact and the skill analyzes the legacy content, maps it to the framework, and runs the design process only for gaps.
---

# generate-archdoc

You are populating `docs/ARCH/index.html`. This is the Plan-phase driver activity owned by the `architect` agent.
...
**e. Apply the ports.** Fill `<section>` blocks in `docs/ARCH/index.html` with classified+refined content. Add Mermaid diagrams where the source had only prose. Add a reference at the bottom of ARCH's Open Questions / Appendix: "Source documents: [docs/archive/...]"
...
Update `docs/ARCH/index.html` in place. Preserve the head, stylesheet link, and Mermaid loader. Each section is a `<section data-section="<name>">` block.
