...
If the config line is still the unfilled placeholder, **default to `stop-at-merge`** and tell the user: *"No delivery-autonomy methodology set — defaulting to stop-at-merge. Run `/setup-tracker` or edit the Delivery autonomy line in process/WORKFLOW.md to change it."*
...
- Otherwise take the active milestone from `process/cairn/milestones/` (status `in-progress`), or the next `planned` one.
...
- **Tracker wired:** `process/cairn/config.yml` must exist. If missing, tell the user to run `/setup-tracker` first.
