...
### Chart-local ramp (PT-61 — issue flow over time) — retired 2026-09-05 (PT-85); lines + values ratified 2026-09-08 (PT-92)

**Retired 2026-09-05 (PT-85, architect ruling 586af1f).** The 6-token ordinal ramp this section originally documented (`--chart-flow-backlog`…`--chart-flow-cancelled`, one per `STATUS_ORDER` status) is **gone from `app.css` and every `variants.css` variant** — implementation-lead removed all six, in both modes, in the same commit as the ruling. Reason: PT-85 replaced the issue-flow chart's cumulative status-stack view with a throughput view (bars: opened/closed per period; line: WIP) — three **categorical** series that need to be told apart by hue at a glance, not an **ordinal** progression through one hue family. Reusing 3 of the 6 retiring steps (implementation-lead's first proposal) was rejected for exactly that reason: 3 shades of one hue is the wrong encoding for a contrast a reader has to make instantly. `scripts/cairn/tests/test_chart_variant_ramps.py` (the per-variant ordinal guard) and `ChartRampOrdinalValidationTests` in `test_dashboard_chart_ramp.py` were retired alongside — there is no ordinal ramp left to guard. The history below (re-stepping rationale, the original 6 values, validator evidence) is kept as a design-history record, not as current token state.

**What replaced it: 3 new dedicated categorical tokens**, `--chart-flow-opened` / `--chart-flow-closed` / `--chart-flow-wip`, declared in `app.css` (the `yellow`/base values below) and — **since PT-118, 2026-09-09, see Chart Color now drives flow + counter below** — re-derived per Chart Color variant, not the fixed, identical-everywhere values this paragraph originally shipped. Values placed at 3 new OKLCH points independently constructed for this purpose (not sourced from the dataviz skill's `palette.md` swatches verbatim — every one of that reference palette's 8 named colors measured within 25° hue of some `--chart-role-*` hue in at least one mode, making "reuse a literal palette.md swatch" and the pairwise/role-collision distinctness floor below jointly unsatisfiable on this page's existing role palette):
...
| `--chart-flow-wip` | `oklch(0.62 0.100 200.8)` | `oklch(0.60 0.097 200.8)` | WIP line (point-in-time, end of period) — **chroma re-authored 2026-09-09 (PT-118), see below; was `0.15`/`0.13`** |

**Distinctness thresholds** (architect's pre-registration, PT-85 issue comment 9336826): pairwise hue separation **≥60°** between the three flow tokens; **≥25°** hue separation from every `--chart-role-*` hue used elsewhere on the same dashboard page (role hues: 47.5°/76.8°/105.0°/165.5°/236.2°/252.0°/300.0°/346.3°). The three hues above (16.9°/135.2°/200.8°) sit at the widest gaps between adjacent role hues, **unchanged by PT-92** — every token change below is L/C-only, hue-preserving, so both hue-based guards are untouched. Guarded mechanically by `scripts/cairn/tests/test_flow_series_distinctness.py` (pairwise ΔH ≥60° and role ΔH ≥25°, both modes, read from `dashboard/src/variants.css`).

**PT-92 resolution (ux-designer, 2026-09-08).** PT-85's values were interim pending ratification; PT-92 also moves the chart from filled `Bars` (opened/closed) + one `Spline` (wip) to **three overlapping line marks**, per Mosko's board comment and the architect's gate-1 ruling that lines alone don't fix the "hidden series" defect — coincident values (measured live: ≥2 of 3 series equal on ~18% of points) still fully occlude unless each series carries its own non-color-only identity at *every* x. Disposition, value by value:

- **Ratified as-is:** `--chart-flow-opened` (both modes), `--chart-flow-wip` (both modes), `--chart-flow-closed` dark (`oklch(0.65 0.15 135.2)`, 5.69:1 against the dark `--card` already). PT-85's architect-suggested hue placements and independently-tuned L/C already clear every hard floor (role-collision, pairwise, lightness/chroma band, normal-vision, contrast) with real margin; re-deriving them would only reintroduce risk for no gain.
- **Replaced:** `--chart-flow-closed` **light mode only**, `oklch(0.68 0.18 135.2)` → `oklch(0.62 0.18 135.2)` (C and H unchanged). PT-85 accepted a 2.72:1-vs-white contrast WARN here on the strength of the dataviz skill's stated relief for that band ("the chart always carries a text legend + tooltip") — a relief calibrated for a *filled bar's* area, not a *dashed line's* stroke. This ticket's own coincidence fix draws `closed` as a dashed line on top of `opened`'s solid line (see **Line treatment (PT-92)** below), so at any shared x the reader resolves `closed` from the dash gaps alone — exactly the low-area, high-precision case the WARN's bar-shaped mitigation doesn't cover. Measured (own OKLCH→sRGB + WCAG luminance reimplementation; the dataviz skill's `validate_palette.js` was unreachable this session — same standing caveat as the Categorical role palette section below): old `#63af2a` vs `#ffffff` = **2.72:1**; new `#509c06` vs `#ffffff` = **3.44:1** in both the float and quantized-hex models (checked separately per the PT-69 "two-model rule" precedent) — clears the WCAG 1.4.11 non-text-contrast floor outright, no WARN, no legend dependency. Trade flagged, not hidden: a deuteranopia re-simulation (Machado/Oliveira/Fairchild 2009) shows the darkened `closed` sits marginally closer to `opened` under deutan simulation than the old value did, while moving further from `wip` by a larger margin; light mode carried no CVD WARN before this change (unlike dark mode's already-accepted opened/closed WARN at ΔE 7.1), so this isn't crossing into a new-risk band, but **re-run through the actual skill script before final lock**, per this doc's standing "flagging the method, not asserting precision" posture.

**Line treatment (PT-92, ux-designer).** Full spec (stroke weight, dash/marker pattern per series, z-order, legend order, WIP-on-axis remedy) is recorded on the PT-92 issue file rather than duplicated here — this doc tracks token *values*, not per-feature rendering specs. Summary: `opened` solid stroke (bottom), `closed` dashed stroke on top of it (dash gaps expose `opened` at any coincidence), `wip` dotted stroke on top of both (also resolves WIP reading as the x-axis rule when wip=0); graduated per-series point markers (largest→smallest, opened→closed→wip) so an exact-coincidence point still shows a ring of each underlying color, not just the topmost. Legend fixed order: Opened, Closed, WIP.

### Chart Color now drives flow + counter (PT-118, 2026-09-09)

**Why this exists.** Mosko: "chart-color in the top right settings no longer seems to take effect" — true. Both dashboard charts had moved to the fixed families above (this section, and **Counter-type palette** below), so the Chart Color setting changed `--chart-1..5` (consumed by nothing) and nothing else. Decision (Mosko): make it drive both charts rather than retire it.

**Mechanism: rotate, then map — both steps mechanical, in `gen_variants.py`, no per-variant hand authoring.**
1. **Rotate.** `Δh(variant) = variant["chart-1"].h − yellow["chart-1"].h` (98.111°, the same for both modes — confirmed mode-invariant across all 23 variants), read from already-vendored `variants.json`. Apply `Δh` to every hue in `--chart-flow-{opened,closed,wip}` and `--chart-counter-{input,cache-write,cache-read,output}`, both modes, holding each token's own ratified `L` and `C`. A uniform rotation is a rigid rotation of the OKLab a/b plane — an isometry — so it preserves PT-92's pairwise (≥60°) and role-collision (≥25°) hue-difference guards **exactly**, for every variant, by construction; no per-variant re-tuning of hue placement is needed or done.
2. **Map.** Rotation alone is not sufficient: sweeping all 23 variants × 2 modes × 7 tokens (322 checks), 61 (19%) land outside the sRGB gamut after rotation — chroma is fixed, but the gamut boundary is hue-dependent, so a fixed chroma that was safely inside sRGB at the original hue is not guaranteed to stay inside it at a rotated one (median overage 22.6%, worst 53%). This is the **common case, not an edge case** — the generator gamut-maps every result: hold `L` and `h`, binary-search the largest in-gamut `C` (an *unclamped* OKLCH→linear-sRGB check; the existing helper clamps its return value, which makes a gamut test pass vacuously by construction — a second, unclamped implementation is required to detect the thing being tested). `yellow` is `Δh = 0`, and every one of its tokens is already in-gamut (see the wip re-authoring below), so mapping is a no-op for it — byte-identity against today's ratified values is structural, not a coincidence.

**`--chart-flow-wip`'s chroma was already unrenderable before this ticket — re-authored, not grandfathered.** The gamut check surfaced that the ratified `oklch(0.62 0.15 200.8)` / `oklch(0.60 0.13 200.8)` sit 29.7%/21.5% outside sRGB at their own hue — browsers were already silently gamut-mapping every render of the default variant to some engine-specific approximation; nobody has actually seen the authored numbers on screen. Re-authored to the largest in-gamut chroma at the same `L`/`h`, with ~5% headroom below the exact edge (rounding/engine differences shouldn't reopen this on a different renderer): light `0.15 → 0.100`, dark `0.13 → 0.097` (table above). Contrast actually improves (3.49:1 light / 4.63:1 dark vs `--card`, both comfortably over the 2:1 floor); pairwise/role-collision guards are hue-only and untouched. This is now wip's ratified value — `Δh=0`'s byte-identity claim is against this corrected baseline.

**Role family (`--chart-role-1..8` + 3 guards) stays fixed — deliberately excluded from rotation.** Role hue is an identity mapping, not a stylistic one: PT-79 cross-checked each hue once against `--primary`/`--destructive` so a role segment can't be mistaken for a brand or danger signal, and PT-69 already ruled that a token's meaning must not silently change depending on which control the user is looking at (the Paused-badge/`--chart-2` collision). Rotating role hue with an unrelated Chart Color preference would reopen that defect at a wider scale, across two views (cost and tokens) of the same dashboard block. The mechanics agree independently: rotating role would put 146 of 368 checks (40%) outside sRGB, at chroma up to 0.24 — nearly double the flow/counter failure rate. **Acceptance consequence, stated because it's the one place the decision is visible:** switching Chart Color changes the token chart's *tokens* (counter-colored) view; its *cost* (role-colored) view does not change. `--chart-1..5` are retained deliberately as this rotation's hue **source** (`Δh` is read from them) — that is now their only remaining consumer; do not delete them as unused.

**Guards, re-verified mechanically after mapping, not inferred from the rotation argument alone** (measured across all 322 flow+counter combinations): categorical/pairwise OKLab ΔE, min **0.0705** before and after mapping, zero pairs below the 0.06 floor — chroma reduction moves a point *radially*, not angularly, so this can't be assumed invariant like the pure hue-difference guards and was measured, not asserted; contrast ≥2:1 vs both `--card` surfaces (two-model), zero failures; in-gamut, hard fail on the unclamped check (with a synthetic out-of-gamut control proving the check can fail at all — a clamp-based gamut test that never fails is worse than no test). **Fallback, reserved for a future failure, not used today:** if a variant×mode still fails contrast or the categorical floor after gamut-mapping, nudge that token's `L`/`C` only (hue fixed), recorded per variant — the same shape as the existing `--chart-flow-closed` light-mode nudge, never a hue re-placement.
...
**Since PT-118 (2026-09-09): re-derived per Chart Color variant.** The values below are the `yellow`/base set only — every other Chart Color variant rotates + gamut-maps this family the same way it does `--chart-flow-*`; see **Chart Color now drives flow + counter** above for the mechanism, guards, and why the role family (unlike this one) stays fixed.
...
| **Chart Color** | `data-cairn-chart` | `--chart-1..5` (re-derived per variant; **since PT-118, also this dimension's rotation source**) + `--chart-flow-opened/closed/wip` + `--chart-counter-input/cache-write/cache-read/output` (rotated + gamut-mapped per variant since PT-118 — see **Chart Color now drives flow + counter**; `--chart-role-1..8` deliberately excluded, stays fixed as an identity mapping) | `yellow` | **Yellow** (default) + **the full reference set** — expanded 2026-08-29 (Mosko), the ≤5 cap is lifted; superseded my earlier curated 3 |
