...
### Chart-local ramp (PT-61 — issue flow over time) — retired 2026-09-05 (PT-85)
...
| `--chart-flow-closed` | `oklch(0.68 0.18 135.2)` | `oklch(0.65 0.15 135.2)` | issues closed per period |
| `--chart-flow-wip` | `oklch(0.62 0.15 200.8)` | `oklch(0.60 0.13 200.8)` | WIP line (point-in-time, end of period) |

**Distinctness thresholds** (architect's pre-registration, PT-85 issue comment 9336826): pairwise hue separation **≥60°** between the three flow tokens; **≥25°** hue separation from every `--chart-role-*` hue used elsewhere on the same dashboard page (role hues: 47.5°/76.8°/105.0°/165.5°/236.2°/252.0°/300.0°/346.3°). The three hues above (16.9°/135.2°/200.8°) sit at the widest gaps between adjacent role hues. Validated in both modes via the dataviz skill's `validate_palette.js` (Lightness band, Chroma floor, CVD separation, normal-vision floor, contrast vs. `--card`) — all checks pass in both modes, with one WARN each (CVD separation in the labeled-mitigation 6–8 band, satisfied here since each series also carries a direct legend label, not color alone). Guarded mechanically by `scripts/cairn/tests/test_flow_series_distinctness.py` (pairwise ΔH ≥60° and role ΔH ≥25°, both modes, read from `dashboard/src/variants.css`) — interim pending ux-designer ratification of the exact hex values (PT-92, filed).
