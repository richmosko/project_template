...
**Embed dedup, ruled 2026-08-29 (Mosko's live-test finding: two triggers show when the board renders embedded in the dashboard).** The embedded board hides its own trigger entirely; the host dashboard page's trigger governs. Both surfaces already read/write the same origin-global `cairn.theme` key, so the embedded trigger would be fully redundant — same state, same control, same visual position, on screen twice. "Hides" means **not rendered at all when embedded**, not `display:none` — a hidden-but-present control still sits in the DOM/tab order, which is a real keyboard-navigation defect (a sighted mouse user never notices a `display:none` ghost control, a keyboard user tabbing through the embedded board does). Detection mechanism (which existing embedded-mode signal the board already has, e.g. the one `board-logic.js`'s `viewStateKey` handling implies) is implementation-lead's call, not a design one — this rules the *outcome* (exactly one trigger visible and reachable per screen, always the host's), not the *how*.
