...
- `allocate_and_create_issue(data_dir: Path, fields: dict) -> Path` — computes `max(numeric suffixes across data_dir/issues/*.md and archived_issue_paths(data_dir)) + 1` and atomically creates `data_dir/issues/<prefix>-<n>.md` via `O_CREAT|O_EXCL`, retrying `n+1` on collision up to 50 times. `fields` supplies everything except `id`/`created`/`updated`, which this function fills in. `prefix` comes from `load_config(data_dir)["prefix"]`.
- `archived_issue_paths(data_dir: Path) -> List[Path]` — PT-50: every archived issue `.md` file, both layouts (`data_dir/archive/issues/*.md`, listed first, then the legacy flat `data_dir/archive/*.md`). THE single point every archived-issue read site routes through — see [Archive](#archive) in the spec.
